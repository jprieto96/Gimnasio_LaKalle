package negocio.clase.mostrarClasesPorEntrenador;
...
public class TestNegocioMostrarClasesPorEntrenador {
	...
	public void before() {
		...
		idEntrenador = daoPersonal.create(new TEntrenador("Pepe", ejDNI.toString(), ejCB.toString(), ejTFNO.toString(), 2300, 4, true), Evento.ALTA_ENTRENADOR);
	...
	public void testListarClasesPorAulaCorrecto() {
		List<TClase> clases = saClase.mostrarClasesPorEntrenador(idEntrenador);
		for (TClase c : clases) {
			assertNotNull(c);
			assertEquals(c.getIdEntrenador(), idEntrenador);
		}
	}
